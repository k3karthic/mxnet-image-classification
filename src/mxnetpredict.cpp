...
MXNetPredict::MXNetPredict(QString pathPrefix, DevType dt, int dev_id) {
  auto symbolName = "resnet-152-symbol.json";
  auto paramName = "resnet-152-0000.params";
...
MXNetPredict::getPredictions(std::vector<mx_float> image_data, int n) {
  ...
  MXPredCreate(this->symbolFile.constData(), this->paramFile.constData(),
               static_cast<size_t>(this->paramFile.length()), this->dev_type,
               this->dev_id, num_input_nodes, input_keys, input_shape_indptr,
               input_shape_data, &handle);
